...
public class DefaultMQProducerImpl implements MQProducerInner {
        ...
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    //把生产者的instanceName改为进程号
                    this.defaultMQProducer.changeInstanceNameToPID();
                }
                //整个jvm中只存在一个MQClientManager实例，维护一个MQClientInstance缓存表：factoryTable
                //同一个clientId只会创建一个MQClientInstance
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);

                //向MQClientInsatnce注册，将当前生产者加入到MQClientInsatnce的内存producerTable中
     ...
     * 对发送结果进行封装返回
     * @param msg 消息
     * @param communicationMode 通信模型
     * @param sendCallback 发送回调
     * @param timeout 发送超时时间
     * @return
     * @throws MQClientException 当client发生异常
     * @throws RemotingException 当远程请求发生异常
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        endTimestamp = System.currentTimeMillis();
                        //发送异常则调用mqFaultStrategy.updateFaultItem方法，更新broker可用性信息，继续循环
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        //缓存中存在topic的路由信息则返回，不存在则向nameServer查询
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            //如果nameserver查询不到，再次尝试用默认主题defaultMQProducer#createTopicKey去查询
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (null == brokerAddr) {
            //如果缓存中不存在broker信息则从nameserver主动更新
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        SendResult sendResult = null;
        //为消息添加属性， TRAN_MSG 和PGROUP ，分别表示消息为prepare 消息、消息所属消息生产者
        ...
        switch (sendResult.getSendStatus()) {
            //如果消息发送成功， 则执行TransactionListener #executeLocalTransaction 方法，该
        ...
        try {
            //结束事务。根据第二步返回的事务状态执行提交、回滚或暂时不处理事务。(暂时不处理的情况，由事务消息状态回查线程 回查时再决定是否提交或回滚)
    ...
    public void endTransaction(
        ...
        switch (localTransactionState) {
            ...
            case ROLLBACK_MESSAGE:
                ...
                break;
            case UNKNOW:
                //结束事务，但不做任何处理。(事务消息的提交与回滚通过事务消息状态回查时再决定是否提交或回滚。)
